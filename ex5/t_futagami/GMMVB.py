...
class GMMVB:
    ...
    def visualize_1d(self, X):
        ...
        # Create a grid for plotting the GMM PDF
        x_min, x_max = X[:, 0].min() - 1, X[:, 0].max() + 1
        xx = np.linspace(x_min, x_max, 1000).reshape(-1, 1)
        pdf = np.zeros(xx.shape[0])
        for k in range(self.K):
            rv = multivariate_normal(mean=self.m[k], cov=la.pinv(self.nu[k] * self.W[k]))
            pdf += self.alpha[k] * rv.pdf(xx).ravel()
        # Plot the GMM PDF
        ax.plot(xx.ravel(), pdf, color="black", linewidth=1, label="GMM PDF")
        ax.set_xlabel("x")
        ax.set_title("data1")
        ax.legend()
    ...
    def visualize_2d(self, X):
        ...
        for k in range(self.K):
            rv = multivariate_normal(mean=self.m[k], cov=la.pinv(self.nu[k] * self.W[k]))
            pdf += self.alpha[k] * rv.pdf(grid).reshape(xx.shape)
        # Plot the contours of the GMM PDF
        ...
        fig.colorbar(sm, ticks=cs.levels, ax=ax, label="GMM PDF")  # Add color bar
        ax.set_xlabel("x1")
        ax.set_ylabel("x2")
        ax.legend()
        ax.set_title("data3")
