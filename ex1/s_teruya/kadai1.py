...
def stft(data: np.ndarray, rate: float, trange: float = TRANGE, overrate: float = OVERRATE):
    ...
    for i in range(int(len(data)/rate/trange)):
        ...
        try:
            if i == 0:
                ...
                shortWave = data[int((i-overrate)*trange*rate)                                 :int((i+overrate+1)*trange*rate)]
        except:
            shortWave = data[int((i-0.01)*trange*rate):]
        # 変換
        # window = np.sin(np.linspace(0, 2*np.pi, len(shortwave)+2)[1:-1])    # sin窓
        window = get_window('hann', len(shortWave) +
                            1)[1:]                   # hann窓
...
def istft(spektrem: np.ndarray[np.ndarray], rate: float, overrate: float = OVERRATE, limit=None):
    ...
    for i in range(len(spektrem)):
        if i == 0:
            ...
            fixPart = np.real(ifft(spektrem[i]))*window   # IFFTで逆変換
            overline = int(len(fixPart)*2*overrate /
                           (1+2*overrate))    # 重なり部分は足す
...
if __name__ == "__main__":
    ...
    if len(data.shape) == 2:
        ...
        if onlyOne:
            ...
            plt.subplot(312)
            # plt.specgram(data1, Fs=r)
            spek_t, f, spektrem1 = stft(data1, r)
            plt.pcolormesh(spek_t, f, np.log1p(
                np.abs(spektrem1.T)), shading='auto')
            plt.xlabel('s')
            plt.ylabel('Hz')
            plt.subplot(313)
            fix_t, fixdata1 = istft(
                spektrem1, r, limit=1.1*np.abs(data1).max())
            plt.plot(fix_t, fixdata1)
            fixdata1_int16 = np.int16(
                fixdata1 / np.max(np.abs(fixdata1)) * 32767)
            ...
            plt.subplot(324)
            # plt.specgram(data2, Fs=r)
            spek_t, f, spektrem2 = stft(data2, r)
            plt.pcolormesh(spek_t, f, np.log1p(
                np.abs(spektrem2.T)), shading='auto')
            plt.xlabel('s')
            plt.ylabel('Hz')
            plt.subplot(326)
            fix_t, fixdata2 = istft(
                spektrem2, r, limit=1.1*np.abs(data2).max())
            plt.plot(fix_t, fixdata2)
            if len(fixdata1) == len(fixdata2):
                fixdata1_int16 = np.int16(
                    fixdata1 / np.max(np.abs(fixdata1)) * 32767)
                fixdata2_int16 = np.int16(
                    fixdata2 / np.max(np.abs(fixdata2)) * 32767)
        ...
        plt.subplot(312)
        # plt.specgram(data1, Fs=r) # ほぼSTFTかも
